...
const API_NINJAS_URL = "https://api.api-ninjas.com/v2/recipe";
...
function buildSearchParams(message: string) {
  ...
  const title = normalizeText(message).slice(0, 80);
...
function toRecipe(raw: unknown): ApiNinjasRecipe | null {
  ...
  const ingredients = Array.isArray(obj?.ingredients)
    ? obj.ingredients.map((v) => String(v).trim()).filter(Boolean).slice(0, 14)
    : [];
...
export async function fetchApiNinjasRecipes(message: string, limit = 2): Promise<ApiNinjasRecipe[]> {
  ...
  const res = await fetch(`${API_NINJAS_URL}?${params.toString()}`, {
    headers: {
  ...
  const recipes = Array.isArray(data)
    ? data.map(toRecipe).filter((r): r is ApiNinjasRecipe => Boolean(r)).slice(0, Math.max(1, Math.min(limit, 4)))
    : [];
